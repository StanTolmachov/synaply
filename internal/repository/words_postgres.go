...
WHERE user_id = $1 
ORDER BY 
    -- Приоритет 1: Слова на повторение, чье время УЖЕ ПРИШЛО (due <= сейчас)
    CASE WHEN state != 0 AND due <= NOW() THEN 1 
    -- Приоритет 2: Абсолютно новые слова
         WHEN state = 0 THEN 2 
    -- Приоритет 3: Слова на повторение, время которых еще не пришло
         ELSE 3 
    END ASC,
    ...
    -- Внутри группы новых слов сортируем по дате добавления
    created_at ASC
LIMIT 10;
	`
